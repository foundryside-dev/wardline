...
## 2026-06-21 - Add Third-Party Deserialization Sinks to Core Taint Mapping
**Vulnerability:** The static analyzer was missing `dill.load`, `dill.loads`, `jsonpickle.decode`, `joblib.load`, and `torch.load` in its `_SERIALISATION_SINKS` mapping, potentially leading to false negatives when tracking untrusted data flowing into these third-party deserialization functions.
**Learning:** Similar to the PyYAML findings, functions listed in rule specifications (like `_SINK_SPECS` for PY-WL-106) must also be properly categorized in the core taint propagation logic (`_SERIALISATION_SINKS`) to ensure the analyzer sheds validation provenance correctly (converting output to `UNKNOWN_RAW`). Conditionally unsafe sinks (like `numpy.load` with `allow_pickle=True`) are excluded to prevent false positives.
**Prevention:** When defining new deserialization sinks in rule definitions, always verify and include unconditionally unsafe sinks in core propagation mappings like `_SERIALISATION_SINKS`.
