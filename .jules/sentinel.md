...
**Vulnerability:** The static analyzer's taint tracking was missing `dill.load`, `dill.loads`, `jsonpickle.decode`, `joblib.load`, `torch.load`, and `numpy.load` in its `_SERIALISATION_SINKS` mapping, potentially leading to false negatives when tracking untrusted data flowing into these dangerous deserialization functions.
**Learning:** Functions that act as deserialization sinks need to be categorized in the core taint propagation logic (`_SERIALISATION_SINKS`) to ensure the analyzer correctly sheds validation provenance (converting output to `UNKNOWN_RAW`). They were defined in the rule (`untrusted_to_deserialization.py`), but missed in the actual variable taint processing.
**Prevention:** When adding new sinks to rule definitions, always cross-reference and verify if they need to be added to core propagation mappings like `_SERIALISATION_SINKS` or `_PROPAGATING_BUILTINS` to accurately track taint flow.
