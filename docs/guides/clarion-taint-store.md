...
The Clarion taint store is an **opt-in** persistent back-end for taint facts.
When configured, each `wardline scan` writes per-entity taint facts to Clarion;
...
!!! info "Never required"
    There is **no Clarion dependency by default**. `wardline scan` boots,
    analyzes, writes findings, and gates without Clarion present. The store is
...
Then pass `--clarion-url` to point at your Clarion instance:

```console
$ wardline scan src/ --clarion-url http://localhost:9100
```

The same flag works for the MCP server:

```console
$ wardline mcp --root . --clarion-url http://localhost:9100
```

Without the extra installed, `--clarion-url` is a hard error (exit `2` on the
CLI; a `not-reachable` result block in the MCP response so the scan payload
survives).

## Authentication

Wardline reads an auth token from `WARDLINE_CLARION_TOKEN` — set it in your
environment or place a `WARDLINE_CLARION_TOKEN=...` line in a `.env` file at
the scan root. An already-set environment value always wins; the `.env` read
never silently overrides it.

The token value must match the one the Clarion operator configured in
`serve.http.identity_token_env` on the Clarion side. Wardline never reads
...
- Pass the finding's `qualname` as `sink_qualname` and a fresh fact is served
  from Clarion without re-scanning the file at all.
...
Clarion only reports the live hash. If the file has changed since the last
...
Clarion is never on the critical path. The full degradation matrix:

| Condition | Behavior |
|---|---|
| Clarion absent or unreachable | warn and continue; scan exits normally |
| Clarion responds `WRITE_DISABLED` | skip write silently; scan continues |
...
| Hard error (missing extra, bad URL scheme, 4xx) | CLI exits `2`; MCP returns scan with `not-reachable` Clarion block |

The CLI-vs-MCP distinction matters for hard errors: a hard `ClarionError` on
...
the error, so the scan result is returned with a `clarion` block indicating the
...
hash per file) and POSTs them to Clarion. This is fail-soft, but it is a real
per-scan cost in a tight agent loop — if Clarion is slow or distant, factor
