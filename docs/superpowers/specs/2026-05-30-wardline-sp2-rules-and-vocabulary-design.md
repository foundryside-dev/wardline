...
- **~~Known limitation — L2 `match`-statement taint~~ RESOLVED 2026-05-30 (own unit after SP2c).** `compute_variable_taints` originally had no `ast.Match` branch, so assignments and capture-pattern bindings inside `match`/`case` arms did not propagate taint — the **assign-in-arm → return-the-var-later** path (`@trusted def f(p): x = 1; match p: case 1: x = read_raw(p); ...; return x`) was a fail-open under-taint PY-WL-101 missed. Closed by `_handle_match` + `_collect_pattern_targets` (`variable_level.py`): mirrors the `_handle_if`/`_handle_try` snapshot-branch-join (`taint_join` merge incl. the no-match fall-through) and binds every capture-pattern target to the subject's taint (conservative whole-subject over-approximation — never under-taints). The leak now resolves to `MIXED_RAW` and fires PY-WL-101. Plan: `docs/superpowers/plans/2026-05-30-wardline-l2-match-handling.md`. (SP2c's `compute_return_taint` had already closed the *direct-call-return-in-arm* half.)
