...
- **Known limitation — L2 `match`-statement taint (next unit, found during SP2c review).** `compute_variable_taints` (`variable_level.py`) has no `ast.Match` branch in `_process_stmt`, so assignments and capture-pattern bindings inside `match`/`case` arms do not propagate taint to locals. SP2c's `compute_return_taint` fix closed the *direct-call-return-in-arm* path (`case 1: return read_raw(p)` is now caught), but the **assign-in-arm → return-the-var-later** path is still a fail-open under-taint reachable through PY-WL-101: e.g. `@trusted def f(p): x = 1; match p: case 1: x = read_raw(p); ...; return x` resolves `x` to INTEGRAL and PY-WL-101 misses the leak. This is a pre-existing SP1 L2 gap (not an SP2c regression; self-hosting is unaffected — `src/wardline` is undecorated). **Fix is its own focused unit** (mirror `_handle_if`/`_handle_try` branch-merge + add capture-pattern target extraction for sequence/class/mapping/`as` patterns and guards) — NOT folded into SP2d's vocabulary descriptor.
