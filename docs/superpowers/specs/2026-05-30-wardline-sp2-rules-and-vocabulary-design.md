...
|---|---|---|---|
| **PY-WL-101 untrusted-reaches-trusted** | taint-flow | A `@trusted(level=L)` function whose returned value's **actual** L2 taint is less-trusted than `L` — i.e. untrusted data flows out of a function that claims to produce trusted data, with no validation boundary. | Walk the function's `return` statements; look up the returned name in `function_var_taints[qualname]`; compare its `TRUST_RANK` against `L`. Less-trusted ⇒ DEFECT. |
