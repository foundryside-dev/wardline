...
**SP1d forward note (HARD, from SP1c):** SP1c's minimum-scope edge builder dropped `.old`'s `self.method()` resolution (it needs the caller's enclosing class, which `Entity` doesn't store). Omitting it **under-taints** (a missed `self.raw_method()` edge leaves the caller looking more trusted than reality — the false-negative direction). SP1d's full callgraph **must** port `resolve_same_class_call_fqn` (from `.old/scanner/import_resolver.py`) and resolve self/cls method calls against class scope, or self-method flows are lost everywhere, not merely deferred. (Asymmetry to recall: SP1c also substituted `iter_calls_in_function_body` for `.old`'s dead-branch-skipping `iter_reachable_calls` — that one *over*-taints, the safe direction, and needs no recovery.)

**SP1f forward notes (from SP1c):**
- **Serialization-sink vs `stdlib_taint` precedence.** L2's `_SERIALISATION_SINKS` maps `json.load`/`json.loads` → `UNKNOWN_RAW`, while `stdlib_taint.yaml` maps them → `GUARDED` (the only overlap). There is no live collision until SP1f wires `stdlib_taint` into the call-resolution `taint_map`. When SP1f does so, the **conservative `UNKNOWN_RAW` wins** (fail-closed posture) — the body-local serialization heuristic overrides the cross-module summary.
- **Restore the external-dotted-taint channel + test.** SP1c dropped `.old`'s manifest `dependency_dotted_map`/`dependency_local_prefixes` L2 params (and `.old`'s `test_dependency_taint_resolution`). SP1f's engine should canonicalize `stdlib_taint` (and any config-declared external taints) into L2's `taint_map` (keyed by call-site dotted name), and re-establish a regression test that an external dotted call (e.g. `pd.read_csv`) carries its declared taint.
- **Per-function error boundary (robustness).** L2's `_resolve_expr` recurses structurally; a pathological/generated expression (e.g. a 1000-deep `+` chain) can raise `RecursionError`. The analyzer's per-function driver in SP1f should wrap each function's L2 computation in a fail-closed boundary (on error, treat the function as `UNKNOWN_RAW`) rather than letting one pathological body abort the whole scan. (Mid-recursion handling in L2 itself is fragile and deliberately avoided.)
