...
**Date:** 9 August 2026
**Status:** DESIGN, revision 2 — reworked after a changes-requested ultra review (three independent reviews, nine blocking findings, all addressed; see §16)
...
Three argument kinds drive the generic reader; each is fail-closed on any deviation:
...
`RegistryEntry` gains `kwargs: frozenset[str]` (the declared keyword set per marker) and `arg_kinds: Mapping[str, ArgKind]`, powering both the reader and `PY-WL-130`.
...
- The live false-green (`wardline-4928b75782`, verified): an unknown kwarg or positional arg on `@trusted` silently drops the seed and the scan goes greener with no diagnostic. Two rules ship **before any new marker**: **`PY-WL-130`** (ERROR DEFECT; id provisional) — a builtin marker call carrying an undeclared keyword or any positional argument (not silenced by the builtin-stays-quiet convention; a new rule id is covered by no golden); **`WLN-ENGINE-UNKNOWN-MARKER`** (FACT) — an unrecognised decorator whose resolved FQN root is `weft_markers`/`wardline.decorators`, counted in `decorator_coverage`.
...
|---|---|---|
| **S0 — hardening + cross-product prep** | `PY-WL-130` + `WLN-ENGINE-UNKNOWN-MARKER` (`wardline-4928b75782`); the §4.2 ArgKind grammar (P14); QE prerequisites P1–P13; CODEOWNERS; §13.1 consumer prep | `wardline-5a795253f1` |
