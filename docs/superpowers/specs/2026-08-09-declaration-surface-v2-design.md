...
**Date:** 9 August 2026
**Status:** DESIGN, revision 3 — rev 2 (`ed7bfe86`) was reworked after a changes-requested ultra review (three independent reviews, nine blocking findings, all addressed; see §16), then amended pre-execution with two corrections recorded during the S0 plan reviews: §4.2 gains the registry-owned call-form + splat-reading grammar (with §4.3's `PY-WL-130` charter and §13.2's S0 row aligned to it), and §12's P11 splits into P11a (S0) / P11b (Phase 3 release gate). §16 remains the revision-2 disposition table; the pending P0 re-review covers this revision.
...
**P4 — New axes are new types with empty defaults; existing readers are never widened.** `_read_level`, `LevelArg`, and the three existing markers are untouched. Every new field on `SeedResult`/`FunctionTaint`/`FunctionSummary`/`AnalysisContext` defaults empty. Shipping gate: on a declaration-free tree, the byte-identity golden corpus changes **zero bytes**.
...
- **`LevelArg`** — unchanged, builtins only.
...
- `vocabulary.yaml` → `wardline.vocabulary/v2`; new groups (contracts 2, facets 3, restoration 4, sensitivity 5); a `facets:` section joins `entries:`.
- `REGISTRY_VERSION` → `wardline-generic-3` and `_RESOLVER_VERSION` bump (mandatory — the builtin-only fingerprint short-circuit otherwise serves warm pre-upgrade caches against new seeding). **Consumer-first**: Loomweave's descriptor consumer currently requires `wardline-generic-2`; it ships dual-accept (`generic-2 | generic-3`, shared conformance vectors) **before** wardline bumps (§13.1).
- The live false-green (`wardline-4928b75782`, verified): an unknown kwarg or positional arg on `@trusted` silently drops the seed and the scan goes greener with no diagnostic. Two rules ship **before any new marker**: **`PY-WL-130`** (ERROR DEFECT; id provisional) — a builtin marker whose call **shape** is statically malformed per §4.2: an illegal call form, any positional argument, an undeclared or duplicated keyword, a missing required keyword, or a keyword expansion wardline cannot statically read (value-level problems stay with the reader's fail-closed `None` and `PY-WL-114`; not silenced by the builtin-stays-quiet convention; a new rule id is covered by no golden); **`WLN-ENGINE-UNKNOWN-MARKER`** (FACT) — an unrecognised decorator whose resolved FQN root is `weft_markers`/`wardline.decorators`, counted in `decorator_coverage`.
...
- Shadow discipline extends to the whole new vocabulary (`Contract`/`Field`/`Evidence`/`Sensitivity` and all new markers): exact known-export FQNs only, total rejection under a shadowed root, failing closed to `UNREADABLE`.
...
- The cache serialisation carries it; `_CACHE_FILE_SCHEMA_VERSION` bumps; `_parse_cache_taint`/validation widens to admit the two states **only when accompanied by a witness** — an `UNKNOWN_*` without one is a corrupt file, dropped with a warning (fail-closed, the shipped posture).
...
- `TokenSetArg` + seed-signature generalisation; `_grammar_digest` and `_seed_value_identity` extend in the same commit, with a completeness test (every field of `FunctionTaint`/every arg kind appears in the identity functions).
...
The validator shares the provider's reader — one grammar, recogniser-agreement test (the `invalid_decorator_level` anti-drift construction, replicated or promoted to a public engine-floor API in the same pass).
...
Carries: the sorted `declarations[]` ledger; per-kind `declaration_counts`; declaration debt in the posture (lapsed `expires`, stale dependency pins, record-only claims); the payload-resident HMAC disclaimer + git-as-authorship framing (D3); legible grant-state fields (`trusted_packs`, `trust_dependency_taint`, `strict_defaults`) and the dependency-table digest. **Rollout**: the frozen-contract conformance suite and Warpline's consumer transition are part of the S1 work item — shared golden vectors for v3, verifier dual-accept (`attest-2 | attest-3`) lands in consumers before wardline emits v3, and `docs/contracts/wardline-attest-3.md` documents the migration exactly as the v2 contract doc did. The baseline document's new declaration-digest section is behind a `baseline.yaml` `version: 2` bump with a defined migration: version-1 baselines load unchanged (no declaration section = no comparison), the section is additive, **compared and reported, never suppressible**.
...
Per-kind counts in `resolution_posture.to_dict()` (folded from `WLN-ENGINE-METRICS`, zero new analysis): the histogram gains one counter per declaration group (`contracts`, `facets`, `restoration`, `sensitivity`, `dependency_taint`). **Per-group arming**: a group is *armed* iff any of its consuming rules is enabled; `--fail-on-inert` trips when any armed group recognised zero declarations over the existing five-function floor; nothing clears it. Uplift-only declarations sit in a bucket that never de-inerts a scan. `--strict-defaults` reports the count of declarations it dropped. `coverage_pct` stays per-kind, `None` over empty denominators.

## 12. Verification obligations (shipping gates)

**Program prerequisites (S0):** P1 preview findings reconciled into the corpus harness (manifest `maturity` field; may turn the corpus red on PY-WL-116..126 — budgeted); P2 manifest `kind` + `interaction` fields; P3 per-kind FP rate gates; P4 two-run determinism covers `sentinels/`; P5 the invariant split with `MIXED_RAW` untouched; P6 the `RAW_ZONE` × new-states matrix; P7 canonical orderings pinned everywhere they serialise; P8 provider-fingerprint mutation table + collision pairs + reformat stability; P9 shared-reader recogniser-agreement; P10 builtin/custom malformity asymmetry pinned; **P11a** forward vocabulary skew — a decorator rooted in the vocabulary (`wardline.decorators`/`weft_markers`) that the running engine does not recognise takes no opinion (fail-closed), never crashes, and leaves the `WLN-ENGINE-UNKNOWN-MARKER` FACT (`test_unknown_marker_is_no_opinion_never_a_crash`); P12 inertness denominators decided and pinned per §11.4; P13 waiver ceiling decoupled and re-derived; **P14 the marker-argument grammar of §4.2 (ArgKind registry, call form, keyword-only enforcement) lands with `PY-WL-130` — it is the same change**.

**P11b — unknown evidence token (Phase 3 release gate, not S0):** an unknown `TOKEN_SET`/evidence token in a *known* marker's declared argument makes the **whole** declaration unreadable per P3 — nothing is partially honoured. This is a hard release gate that must land before the first evidence-bearing marker ships, and a LEVEL-token typo test is **not** accepted as proxy evidence for it: P11a's unknown-marker case and P11b's unknown-token case are different obligations with different lifetimes.

**Per-kind gates:** ≥3 attributed sentinels; ≥5 attributed TP specimens; ≥1 interaction specimen + its opposite-label sibling (WL-001's floor: the contradiction/match pair); bidirectional reconciliation clean; per-kind FP ≤5% over ≥10 defects (below that, sentinel-gated and stated); two-run byte identity over fixtures and sentinels; all applicable §10 validation rules with paired specimens; **self-hosting green at `--fail-on ERROR` with zero committed suppressions** (WL-001 meets wardline's own `.get` sites first — if it cannot survive its own source it ships preview/opt-in, never with a first waiver); identity corpus **zero bytes** on the no-declaration path, at most one reviewed rekey per kind; grammar goldens re-frozen once per kind via `regen.py --reason`. WL-001 multi-emit ordinal discrimination (two `.get` on one line → distinct fingerprints) in the rekey/collision suites; a ≥3-access branch-join fixture pins source-order emission. Restoration adds: witness presence/validation tests, cold/warm witness equivalence, the 16-row evidence pin, and the bare-assertion sentinel (maximal declaration, no verified evidence → no uplift + gating DEFECT). CODEOWNERS for the identity corpus lands in S0.
...
1. Loomweave descriptor consumer ships dual-accept `wardline-generic-2 | wardline-generic-3` with shared conformance vectors (in the loomweave repo) — **before** wardline bumps `REGISTRY_VERSION`.
2. `wardline-attest-3` contract doc + golden vectors authored; Warpline/verifier dual-accept (`attest-2 | attest-3`) staged — **before** S1 emits v3.
3. Legis confirms it ignores the unknown `declarations` key today (its stated posture) and receives the wire vectors for lockstep adoption.
...
| **S2 — contracts + sensitivity** | label plane (carrier/convergence matrix first — §15), `Contract`/`Field`/`@schema`/`@sensitive` (weft-markers 0.3.0), `PY-WL-127`/`128` + `.setdefault` shape, the §5.2 algebra, contract-anchored modulation, FN FACTs, contract-edit visibility | `wardline-ac5f22e4f1`, `wardline-a72fd8c917`, `wardline-1c0524c578` |
| **S3 — restoration** (last engine-touching stage) | `TokenSetArg`, body-local evidence checks, §8.1 table, witness carrier + cache schema bump, invariant split + ADR, `taint_join` completion, `RAW_ZONE` matrix, separate posture bucket (weft-markers 0.4.0) | `wardline-b9d70c6a3a` |
...
- **Kwargs on the level-bearing markers** — verified silent seed-drop under version skew.
