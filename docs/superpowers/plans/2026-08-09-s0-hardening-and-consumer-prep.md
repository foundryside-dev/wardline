...
***Rev 3.10 correction — rev 3.9's E501 sweep was incomplete, and it swept the wrong half.*** *It measured only this Step 4.2 implementation block. **Step 1's test block has the same defect**: two of its lines are **106 and 110 characters**, so an implementer typing them verbatim authors two E501s into a repo whose `ruff check` baseline is clean — the exact outcome rev 3.9 exists to prevent, in the step that runs FIRST. Measured on the shipped tree at Task 21 execution (2026-08-13); the implementer caught it independently and applied this note's own parenthesised-continuation remedy, changing no string content, name or assertion. The lesson generalises past this task: **a line-length sweep that inspects only the blocks a reviewer flagged is not a sweep.** Any future revision auditing pasted code for a sibling repo's lint configuration must run the check over **every** fenced block in the task, not the ones already under discussion.*
