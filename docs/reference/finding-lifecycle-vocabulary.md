...
`suppressed` count (`src/wardline/cli/scan.py:468`).
...
| CLI summary line | `src/wardline/cli/scan.py:469` | `… {s.active} active` |
| MCP scan response | `src/wardline/mcp/server.py:906` | `summary.active` |
...
The MCP `summary` block exposes `informational` (`src/wardline/mcp/server.py:914`)
and `unanalyzed` (`src/wardline/mcp/server.py:918`); the agent-summary block mirrors
...
The MCP `scan` gate block exposes `gate.tripped` (`src/wardline/mcp/server.py:921`),
`gate.fail_on_unanalyzed`, `gate.verdict` (`src/wardline/mcp/server.py:925`),
`gate.severity_tripped`, `gate.unanalyzed_tripped`, `would_trip_at`, `reason`,
`evaluated`, and `migration_hint`, opened at `src/wardline/mcp/server.py:920`
...
| (historical) CLI summary | Formerly relabelled the `active` count as "N new". **Corrected to "N active"**. | `src/wardline/cli/scan.py:468` |
...
| --- | --- | --- | --- | --- | --- |
| every finding | `N finding(s)` | `total` (`run.py:51`) | `total` (`server.py:905`) | `total_findings` (`agent_summary.py:139`) | one finding per wire entry |
| live defect | `N active` (`scan.py:469`) | `active` (`run.py:52,393`) | `active` (`server.py:906`) | `active_defects` (`agent_summary.py:140`) | no `suppression_state` key (`finding.py:295`) |
| suppressed (sum) | `N suppressed` (`scan.py:468`) | `baselined+waived+judged` | the three keys | `suppressed_findings` (`agent_summary.py:141`) | `metadata.wardline.suppression_state` (`finding.py:295`) |
| baselined | `N baseline` | `baselined` (`run.py:54`) | `baselined` (`server.py:907`) | `baselined` (`agent_summary.py:143`) | `suppression_state: "baselined"` |
| waived | `N waiver` | `waived` (`run.py:55`) | `waived` (`server.py:908`) | `waived` (`agent_summary.py:144`) | `suppression_state: "waived"` |
| judged | `N judged` | `judged` (`run.py:56`) | `judged` (`server.py:909`) | `judged` (`agent_summary.py:145`) | `suppression_state: "judged"` |
| informational (summary) | (the remainder of `total`) | `informational` (`run.py:62`) | `informational` (`server.py:914`) | `informational` (`agent_summary.py:151`) | facts/metrics |
...
| under-scan | `N file(s) could not be analyzed` | `unanalyzed` (`run.py:70`) | `unanalyzed` (`server.py:918`) | `unanalyzed` (`agent_summary.py:152`) | `WLN-ENGINE-*` facts |
| gate verdict | exit code + `--fail-on` | (`gate_findings`, `run.py:89`; `GateDecision`, `run.py:99`, `verdict` `run.py:108`) | `gate.tripped` (`server.py:921`), `gate.verdict` (`server.py:925`) | `gate.tripped` (`agent_summary.py:155`), `gate.verdict` (`agent_summary.py:158`) | not emitted to Filigree |
