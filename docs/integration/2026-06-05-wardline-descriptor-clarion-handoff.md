# Hand-off: Wardline vocabulary descriptor → Clarion plugin migration

**For:** Clarion (`clarion-1f6241b329`) · **From:** Wardline pre-Rust hardening
...
coupling (Clarion ADR-018) is **complete**. This document is the contract Clarion
consumes to finish the switch. Clarion's reader already exists
(`plugins/python/src/clarion_plugin_python/wardline_descriptor.py`); the two
...
## Resolution of Clarion's two pending assumptions
...
1. **`schema` field — now pinned.** `schema: wardline.vocabulary/v1`. Contract
   ...
   level fields** within the same `schema` (Clarion's reader already ignores
...
2. **File location — resolved.** The **canonical, always-present** descriptor is
   ...
   pin one with `wardline vocab > .wardline/vocabulary.yaml`. Clarion's existing
...
This was **verified live**: Clarion's `wardline_descriptor._state_from_text`
...
## What Clarion does next (out of Wardline scope — `clarion-1f6241b329`)
...
non-event for Clarion. **The native-module migration must not land before this
...
- Clarion reader: `plugins/python/src/clarion_plugin_python/wardline_descriptor.py`,
