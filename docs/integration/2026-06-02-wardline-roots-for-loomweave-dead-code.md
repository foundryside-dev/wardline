# Wardline → Loomweave: trust boundaries as reachability roots (dead-code enrichment)

**From:** Wardline maintainer (John, with Claude)
**To:** Loomweave maintainers
**Date:** 2026-06-02
**Status:** Wardline signal shipped; Loomweave consumption remains additive / non-blocking
**Relates to:** Loomweave `find_dead_code` plan (Part B), the "Root categorisation
tag-emission pipeline" follow-up WP, the shipped SP9 Wardline→Loomweave taint-store
channel (`docs/integration/2026-05-30-wardline-loomweave-taint-store-requirements.md`,
`docs/guides/loomweave-taint-store.md`), ADR-028 (fail-toward-live), ADR-038 (SEI).

---

## 1. Context

Original verification against Loomweave `main` on 2026-06-02 found the root-tag
pipeline absent. Re-checking Loomweave on 2026-06-04 showed the generic dependency
...
Loomweave's `find_dead_code` (forward reachability over `calls`/`imports`
...
(`crates/loomweave-mcp/src/catalogue/shortcuts.rs:59`), reuses the honest-empty
...
Wardline still does **not** write Loomweave's `entity_tags` table. Its shipped
...
channel. Loomweave can either consult this store directly when assembling roots or
...
graph — exactly the entities that otherwise look dead because Loomweave can't see
...
- `@external_boundary` — "external/untrusted data enters here." A function so
  annotated is, almost by definition, invoked by external/framework code Loomweave
...
- **Trust axis only.** Wardline does not know what an `http-route`, `test`,
  ...
  that is Loomweave's plugin-detector job.
- **Roots, not barriers.** The reflection / dynamic-dispatch barrier (flagged
  entities stay live) is Loomweave's own dynamic-dispatch detection. Wardline
...
## 4. Integration mechanism (Loomweave's design call)

A channel already exists: SP9 has `wardline scan --loomweave-url` writing
per-entity, HMAC-signed, blake3-freshness-gated facts into Loomweave's
**`wardline_taint`** store (`crates/loomweave-storage/src/wardline_taint.rs`) —
a table **distinct from `entity_tags`**. Wardline now emits the root hint over
that channel. Two Loomweave-side consumption paths remain possible:
...
2. **Loomweave's tag pipeline ingests a Wardline-derived `entry-point` tag** into
...
- **Critical path = Loomweave's tag-emission pipeline.** It stands alone and must
  not wait for any Wardline work.
- This enhancement is **strictly downstream**: it is inert until Loomweave can
  ingest the signal (§4), and even then it only sharpens the `entry-point` /
  `exported-api` root classes. Treat it as a dependent follow-up on the Loomweave
  WP, never as a blocker.
- A second precondition Loomweave should name explicitly (independent of Wardline):
