...
- **Loomweave** — enrich those findings by reconciling Wardline's Python qualnames to Loomweave entity IDs.
...
**A. (confirm) Batch scan-results ingest.** Confirm `POST /api/v1/scan-results` accepts `scan_source` plus a list of findings, each carrying `file_path, rule_id, message, severity, line_start, line_end, suggestion, metadata`, and preserves `metadata` verbatim. (Your `ScanIngestResponseWeft` implies this exists — just confirming the field set, especially `suggestion` and a free-form `metadata` on the *ingest* path, not only on the stored row.)
...
**C. (confirm) Standalone `file_path` → `file_id` resolution.** Confirm path-anchored ingest resolves `file_id` in **standalone Filigree** (no Loomweave registry backend), so the (Wardline, Filigree) pair composes without Loomweave present — pairwise composability per the composition law (now authoritative at `~/weft/doctrine.md` §4).
...
## 4. Asks for Loomweave

**A. (confirm) qualname → entity-ID reconciliation.** Wardline emits `metadata.wardline.qualname` as the **combined dotted `module.qualified_name`** (e.g. `auth.tokens.TokenManager.issue`) — deliberately matching Loomweave's L7 form to resolve the [ADR-018](../../loomweave/adr/ADR-018-identity-reconciliation.md) deferred clash in Loomweave's favor. Confirm Loomweave reconciles this to an `EntityId` at ingest/enrichment and that the dotted form is what you want (vs. the old `(module, qualified_name)` tuple).

**B. (decision) Transport role under "native" emission.** [ADR-015](../../loomweave/adr/ADR-015-wardline-filigree-emission.md) scoped Wardline→Filigree through Loomweave's `loomweave sarif import` for v0.1, with a native Wardline emitter as the v0.2 retirement. Wardline now intends to ship the **native Filigree emitter directly** (it's the ~1-day path the ADR-015 spike already costed). Consequence: **Loomweave is no longer on the transport path** for the (Wardline, Filigree) pair, and the Wardline→Filigree pipeline asterisk can retire. (That asterisk is now tracked authoritatively as **A-1** in `~/weft/asterisk-register.md`, where it remains **LIVE** as of 2026-06-05 pending the native emitter shipping and verified Loomweave-absent composition.) Loomweave's role becomes pure **enrichment** (entity reconciliation), not a bridge. Please confirm you're happy to (a) keep the SARIF translator as a general-purpose path for *other* SARIF tools, and (b) treat Wardline as a native emitter. This likely warrants an ADR-015 "Revision 2".

**C. (optional, later) Entity associations.** If Wardline later binds findings to Loomweave entities directly (beyond qualname reconciliation), it would use [ADR-029](../../loomweave/adr/ADR-029-entity-associations-binding.md) `add_entity_association` with `content_hash_at_attach`. Not needed for the first cut — flagged so it's not a surprise.
...
- Wardline has **no runtime dependency** on either sibling. `wardline scan` runs and writes `findings.jsonl` standalone; Filigree/Loomweave emission is opt-in (`--filigree-url`, `--loomweave-*`), absent → degrade silently.
...
2. **Loomweave §4.A** — confirm dotted `module.qualified_name` is the reconciliation key you want.
3. **Loomweave §4.B** — agree Wardline ships as a native Filigree emitter and ADR-015 gets a Revision 2 (Loomweave off the transport path, translator stays for other tools).

---

## Round 1 — Loomweave response (2026-05-29) & Wardline reply

**Loomweave decisions: §4.A ✅ (with a normalization contract), §4.B ✅ (asterisk 1 stays live until ship). Decision 1 is Filigree's.** Two route-backs to Wardline; both answered below.

### Reply to §4.A — qualname normalization contract (accepted)

- Wardline's emitter composes **only the module portion**, applying Loomweave's exact `module_dotted_name()` rules (src/ strip, `.py` drop, `__init__.py` collapse, namespace + non-src layouts). **Please send that function**; Wardline vendors it as the single source of truth.
...
- **Proposal — shared qualname conformance corpus.** A committed JSON of `{layout, file, symbol} → expected_qualname` covering the landmines (src/namespace/non-src, `__init__` collapse, nested class, `<locals>`/closure, decorated/overloaded). Both tools run it in their own CI; federation-safe design-review artifact (nothing imports it). Wardline seeds it from Loomweave's function. **Q back: where does it live — Loomweave repo or a neutral suite location?** This converts byte-equality from assumption to test, on both sides.

### Reply to §4.B — transport / asterisk 1 (agreed in full)

Rev 2 rewrites the retirement *mechanism* (Wardline-side native emitter); asterisk 1 stays **live** in `weft.md` §5 under `release:1.1` until the emitter ships and is verified. No deletion on promise. Agreed.
...
1. **Keep the `wardline.core.registry` import surface intact** in the rebuild → Loomweave's plugin startup contract does **not** break (zero-day compat bridge).
...
Asterisk 2 retires when Loomweave's plugin switches to the descriptor (same ship-not-promise discipline). **Q back: can the plugin consume an NG-25 descriptor, and on what timeline?** Until then the import keeps it running. (Descriptor-only-from-day-one is possible but couples our release timelines — plugin must switch before Wardline's first tag — so the compat bridge is recommended.)

### Accepted consequential ADR touches (Loomweave-side)

- **ADR-018** gets its own dated entry: reconciliation entry point becomes "Wardline emits pre-composed `metadata.wardline.qualname`; Loomweave matches via `find_entity`-by-qualname off the Filigree finding" — replacing the file-path composition rule.
...
- **SP1/SP2:** qualname emitter must replicate Loomweave's `module_dotted_name()` byte-for-byte and preserve `__qualname__` verbatim.
...
- **SP4:** prefer a **top-level `fingerprint` field** for Filigree dedup (shared preference with Loomweave).
...
Filigree has two front doors sharing one request body: `POST /api/v1/scan-results` (classic) and `POST /api/weft/scan-results` (Weft vocab; `/api/scan-results` aliases Weft). **Wardline will emit into `/api/weft/scan-results`** — Wardline is a Weft-native citizen and wants the `ScanIngestResponseWeft` envelope. (SP4 emitter targets the Weft endpoint.)

### §3.C / §3.D / §5 — confirmed

Standalone `file_path`→`file_id` via the default `local` registry (composes with Loomweave absent ✓); `scan_source="wardline"` + `WLN-*` stored as-is ✓; severity CHECK is exactly the 5-level set, our client-side map lands clean (fail-soft maps unknown→`info`, which we won't hit) ✓.

### §4.B federation note — agreed

When ADR-015 Rev 2 lands (Loomweave off transport, Wardline native emitter), Filigree adds a one-line note that `scan-results` is a first-class external entry point for native emitters. No objection from Filigree's side; the endpoint doesn't care who calls it. Wardline agrees.
...
- **SP4:** emit to `POST /api/weft/scan-results`; send top-level `fingerprint` (may mirror into `metadata.wardline.fingerprint`); cap `suggestion` ≤ 10 000 chars.
...
| Weft scan-results endpoint as native entry point | ✅ Wardline emits to `/api/weft/scan-results` |
| Loomweave qualname reconciliation (dotted, pre-composed) | ✅ agreed — pending `module_dotted_name()` handover + conformance corpus |
| Native emitter / ADR-015 Rev 2 / asterisk 1 live until ship | ✅ agreed |
| REGISTRY import preserved + NG-25 descriptor (asterisk 2) | ✅ Wardline commits both; pending Loomweave plugin-reader timeline |
| Severity 4→5 map (Wardline-owned) | ✅ confirmed |
| Entity associations (ADR-029) | ⏸ deferred, not first-cut |

**Open loops (non-blocking for SP0):** qualname corpus location (Loomweave repo vs neutral); Loomweave NG-25 plugin-reader timeline.
...
- Standalone `path`→`file_id` via local registry works (Loomweave absent) ✓.

**⚠️ SP4 wire gotchas (exact):**
- Endpoint **`POST /api/weft/scan-results`**. Per-finding path key is **`path`**, NOT `file_path` (sending `file_path` → 400).
...
### Loomweave (Python plugin) — qualname PRODUCER contract fixed; CONSUMER not yet built
