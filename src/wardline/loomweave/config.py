...
WARDLINE_LOOMWEAVE_TOKEN_ENV = "WARDLINE_LOOMWEAVE_TOKEN"


def load_loomweave_token(root: Path) -> str | None:
    ...
    value = os.environ.get(WARDLINE_LOOMWEAVE_TOKEN_ENV)
    if value:
        return value
    env_path = root / ".env"
    ...
    for raw in env_path.read_text(encoding="utf-8", errors="replace").splitlines():
        line = raw.strip()
        if line.startswith(f"{WARDLINE_LOOMWEAVE_TOKEN_ENV}="):
...
def resolve_project_name(root: Path) -> str:
    """Loomweave's project guard handle: the project-root directory name."""
