...
"""Track 4 (T4.3) — the live Loomweave source for the dossier's linkages section.

Wraps the SP9/Track-3 ``LoomweaveClient`` call-graph reads behind the
...
pre-linkage Loomweave, an unknown entity, or an outage yields an honest
...
CONTENT axis is ``FRESH`` because linkages are read live from Loomweave's current index
...
class _LinkageClient(Protocol):
    """The narrow ``LoomweaveClient`` surface this provider needs (so a test double need
    only implement these two; ``LoomweaveClient`` satisfies it structurally)."""
...
class LoomweaveLinkageProvider:
    """A ``LinkageProvider`` (``core/dossier.py``) backed by a live ``LoomweaveClient``.

    ``linkages_http`` is the capability flag (``_capabilities.linkages.http``) detected
    once by the orchestrator — when False, Loomweave does not serve linkages over HTTP
    ...
    def linkages(self, binding: EntityBinding) -> LinkagesSection:
        if not self._linkages_http:
            return LinkagesSection.unavailable("loomweave does not serve HTTP linkages")
        ...
        if callers is None and callees is None:
            return LinkagesSection.unavailable("loomweave linkages unreachable, unknown, or access-denied")
        ...
        if (callers is not None and callers.truncated) or (callees is not None and callees.truncated):
            notes.append("loomweave truncated the linkage list (more neighbours available)")
        return LinkagesSection(
            ...
            content_status=ContentStatus.FRESH,  # read live from the current Loomweave index
