# src/wardline/loomweave/client.py
"""SP9: the dep-light HTTP+JSON client for Loomweave's /api/wardline/* routes.
...
SOFT (the caller degrades to the SP8 re-run) while a 4xx is a LOUD LoomweaveError
...
LoomweaveError); the envelope `code` is surfaced only as a label (``disabled_reason``)
...
class LinkageResult:
    ...
    live from Loomweave. ``neighbours`` are Loomweave entity ids (locators); ``truncated``
    is Loomweave's own pagination flag (surfaced, never silently dropped)."""
...
class LoomweaveClient:
    ...
    def _require_ok(self, resp: Response, path: str) -> dict[str, Any]:
        ...
        if not 200 <= resp.status < 300:
            raise LoomweaveError(
                f"Loomweave rejected {path} ({resp.status}; code={_error_code(resp.body)}): {resp.body}"
            )
        try:
            parsed = json.loads(resp.body) if resp.body else {}
        except json.JSONDecodeError:
            parsed = {}
        # A non-dict 2xx body would be a Loomweave bug, not a Wardline one; degrade to
    ...
    def batch_get(self, qualnames: list[str]) -> list[TaintFactView] | None:
        ...
        for chunk in _chunks(qualnames, self._batch_max):
            ...
            if not 200 <= resp.status < 300 or not isinstance(parsed, list):
                raise LoomweaveError(f"Loomweave rejected batch-get ({resp.status}; code={_error_code(resp.body)})")
    ...
    def batch_get_by_sei(self, seis: list[str]) -> list[TaintFactBySeiView] | None:
        ...
        PROJECT_MISMATCH → None; a 404 (an older SEI Loomweave lacks the route) or other
        non-2xx → loud LoomweaveError (read-skew). SEIs are sent verbatim, never parsed.
        ...
        for chunk in _chunks(seis, self._batch_max):
            ...
            try:
                ...
                parsed = []
            if not 200 <= resp.status < 300 or not isinstance(parsed, list):
                raise LoomweaveError(f"Loomweave rejected by-sei ({resp.status}; code={_error_code(resp.body)})")
    ...
    # Loomweave ADR-038). These are the IDENTITY READ path: they FAIL-SOFT on every
    # non-happy band (a pre-SEI Loomweave 404s the routes / advertises no `sei` cap), so
    # a consumer can detect a non-SEI Loomweave and DEGRADE rather than guess or crash.
    ...
    def _send_json_soft(
        ...
        any other non-2xx (e.g. a pre-SEI Loomweave's 404, a 4xx), or a non-object/bad
        ...
        if not 200 <= resp.status < 300:
            if resp.status != 404:
                logger.warning(
                    "Loomweave identity read returned status %d for path %s: %s",
                    resp.status,
                    path_and_query,
                    resp.body,
                )
    ...
    def capabilities(self) -> dict[str, Any] | None:
        ...
        probe fails for ANY reason (a pre-SEI Loomweave 404s the route). Lets a consumer
        detect a non-SEI Loomweave and degrade rather than guess."""
    ...
    # GET /api/v1/entities/{entity_id}/callers|callees (Loomweave Wave 0 / WS2). HMAC-
    # gated like /api/v1/files; FAIL-SOFT on every non-happy band (a pre-linkage
    # Loomweave 404s the route; an unknown entity 404s; an outage 5xx → None), so the
    ...
    def get_callers(self, entity_id: str, *, limit: int = 50) -> LinkageResult | None:
        """Inbound callers of ``entity_id`` (a Loomweave locator), or None on a soft
        failure / unknown entity / pre-linkage Loomweave. ``entity_id`` is URL-escaped
