# src/wardline/loomweave/identity.py
"""Track 3 (T3.1/T3.2): the SEI-client abstraction.

Carry Loomweave's Stable Entity Identity (SEI) as the OPAQUE handle for cross-tool
bindings, with an honest two-axis status, and degrade gracefully when a Loomweave
instance does not serve SEI. Built against the spec'd wire contract (SEI standard §4 +
Loomweave ADR-038 + the normative fixtures). A real ``loomweave serve`` (the local 1.0.1+
...
``loomweave:eid:<hex>`` token — so this client is verified against a live SEI-serving
Loomweave (the ``loomweave_e2e`` test exercises the ALIVE + opacity path), not only mocks.
A pre-SEI Loomweave (no ``sei`` capability) is the graceful-degrade case.

Stdlib-only by contract: this module MUST NOT import blake3 or any extra, so importing
it never forces the [loomweave] extra and the base package stays zero-dependency.

OPACITY: the SEI is an opaque token (``loomweave:eid:<hex>``). This module NEVER parses,
...
class SeiCapability:
    """Whether a Loomweave instance serves SEI (from GET /api/v1/_capabilities)."""
...
class TaintStoreCapability:
    """Whether a Loomweave instance serves the T3.4 read-by-SEI taint route
    (from GET /api/v1/_capabilities → ``taint_store.read_by_sei``).

    DISTINCT from :class:`SeiCapability`: an older SEI-capable Loomweave (``sei.supported``
    True) predates migration 0006's ``POST /api/wardline/taint-facts/by-sei`` route, so a
    consumer MUST gate on this flag, never infer the route from ``sei.supported`` (Loomweave's
...
class SeiClient(Protocol):
    """The narrow LoomweaveClient surface :class:`SeiResolver` depends on (so mypy checks
    the calls and a test double need only implement these three). ``LoomweaveClient``
...
class SeiResolver:
    """Resolves locators → :class:`EntityBinding` via a LoomweaveClient, honoring
    capability detection and degrading gracefully. The SEI is treated strictly opaque.

    DEFERRED (no T3.1–T3.3 consumer): ``lineage(sei)`` — Loomweave serves it (SEI std §4)
    ...
    def detect(cls, client: SeiClient) -> SeiResolver:
        ...
        that fails for ANY reason (outage / a pre-SEI Loomweave's 404 / malformed body →
