...
def iter_calls_in_function_body(
    ...
    while stack:
        ...
        if isinstance(current, (ast.FunctionDef, ast.AsyncFunctionDef)):
            kw_defs = current.args.kw_defaults
            if kw_defs:
                for kw_default in reversed(kw_defs):
                    if kw_default is not None:
                        stack.append(kw_default)
            defs = current.args.defaults
            if defs:
                for default in reversed(defs):
                    stack.append(default)
            decs = current.decorator_list
            if decs:
                for decorator in reversed(decs):
                    stack.append(decorator)
            continue

        if isinstance(current, ast.ClassDef):
            kwds = current.keywords
            if kwds:
                for keyword in reversed(kwds):
