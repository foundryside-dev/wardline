    ...
    def walk_node(current: ast.AST) -> None:
        ...
        if typ is ast.FunctionDef or typ is ast.AsyncFunctionDef:
            for decorator in current.decorator_list:  # type: ignore[attr-defined]
                walk_node(decorator)
            for default in current.args.defaults:  # type: ignore[attr-defined]
                walk_node(default)
            for kw_default in current.args.kw_defaults:  # type: ignore[attr-defined]
        ...
        if typ is ast.Lambda:
            ...
            return
        if typ is ast.Call:
            append(current)  # type: ignore[arg-type]
