...
def iter_calls_in_function_body(
    ...
    while stack:
        ...
        if isinstance(current, (ast.FunctionDef, ast.AsyncFunctionDef)):
            kw_defaults = [kwd for kwd in current.args.kw_defaults if kwd is not None]
            if kw_defaults:
                stack.extend(reversed(kw_defaults))
            if current.args.defaults:
                stack.extend(reversed(current.args.defaults))
            if current.decorator_list:
                stack.extend(reversed(current.decorator_list))
            continue

        if isinstance(current, ast.ClassDef):
            kw_values = [kw.value for kw in current.keywords]
            if kw_values:
                stack.extend(reversed(kw_values))
            if current.bases:
