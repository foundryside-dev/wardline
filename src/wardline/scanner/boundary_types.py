# src/wardline/scanner/boundary_types.py
"""Engine-floor boundary primitives for the extensible trust grammar (Track 2).

This module holds the data model the grammar is BUILT from — :class:`LevelArg`,
:class:`BoundaryType`, and the builtin :data:`BUILTIN_BOUNDARY_TYPES` — separated
from :mod:`wardline.scanner.grammar` so the engine modules that only need the
primitives (the rules + the decorator provider) can import them WITHOUT pulling in
``grammar`` (which composes the full rule registry via
``grammar.default_grammar``). That separation is what keeps the engine import graph
acyclic: ``grammar`` may import ``rules``, but ``rules``/``decorator_provider`` must
not reach back up into ``grammar``. They import these primitives instead.

``grammar`` re-exports ``LevelArg``/``BoundaryType``/``BUILTIN_BOUNDARY_TYPES`` so
the grammar module stays the single facade for callers that want the whole vocabulary.

Zero-dep: stdlib + ``core.taints`` + ``core.registry`` + the provider's
``FunctionTaint`` only — the same surface ``grammar`` had.
"""
