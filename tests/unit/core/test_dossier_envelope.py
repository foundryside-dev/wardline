...
def test_oversized_core_scalars_are_compacted_and_marked() -> None:
    # Source-controlled identity/shape text is load-bearing but not unlimited: a
    # pathologically long qualname must be compacted rather than bypassing the budget.
    huge = _minimal_dossier(identity=_identity(qualname="m." + "x" * 12_000))
    bounded = bound_to_budget(huge)
    assert bounded.truncation.truncated is True
    assert estimate_tokens(json.dumps(bounded.to_dict(), sort_keys=True)) <= DOSSIER_TOKEN_BUDGET
    assert "core scalar fields compacted" in (bounded.truncation.note or "")
    assert "identity.qualname" in (bounded.truncation.note or "")
    assert "...<truncated>..." in bounded.identity.qualname
