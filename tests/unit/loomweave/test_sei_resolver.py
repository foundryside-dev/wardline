...
class FakeClient:
    """Stands in for LoomweaveClient at the method boundary the resolver uses."""
...
def test_round_trip_carries_opaque_sei_alive() -> None:
    ...
    assert b.sei == "loomweave:eid:0123456789abcdef0123456789abcdef"
...
def test_resolve_identity_status_maps_resolve_sei() -> None:
    ...
    assert alive.resolve_identity_status("loomweave:eid:x") is IdentityStatus.ALIVE
    assert alive_client.sei_calls == ["loomweave:eid:x"]  # opaque token passed verbatim

    orph = SeiResolver.detect(FakeClient(caps=_CAPS_PRESENT, resolve_sei={"alive": False, "lineage": []}))
    assert orph.resolve_identity_status("loomweave:eid:x") is IdentityStatus.ORPHANED

    # capability absent OR soft outage -> UNAVAILABLE (never guess alive/orphaned).
    assert (
        SeiResolver(FakeClient(), SeiCapability(False)).resolve_identity_status("loomweave:eid:x")
        is IdentityStatus.UNAVAILABLE
    )
    soft = SeiResolver.detect(FakeClient(caps=_CAPS_PRESENT, resolve_sei=None))
    assert soft.resolve_identity_status("loomweave:eid:x") is IdentityStatus.UNAVAILABLE
...
def test_resolve_identity_status_never_guesses_orphaned_from_malformed_body() -> None:
    ...
    for malformed in ({}, {"lineage": []}, {"alive": "false"}, {"alive": None}):
        ...
        assert r.resolve_identity_status("loomweave:eid:x") is IdentityStatus.UNAVAILABLE
