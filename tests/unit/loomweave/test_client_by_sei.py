"""T3.4 — LoomweaveClient.batch_get_by_sei (POST /api/wardline/taint-facts/by-sei).
...
4xx -> loud LoomweaveError, mirroring batch_get). The SEI is OPAQUE — sent verbatim in
...
def _client(transport, *, batch_max=2000):
    return LoomweaveClient(
        "http://loomweave.example", secret="s3cr3t", project="proj", transport=transport, batch_max=batch_max
    )


_SEI = "loomweave:eid:deadbeefcafef00ddeadbeefcafef00d"
_BODY = json.dumps(
    [
        {
            "sei": _SEI,
...
def test_batch_get_by_sei_parses_views_and_signs():
    ...
    assert method == "POST"
    assert url == "http://loomweave.example/api/wardline/taint-facts/by-sei"
...
def test_batch_get_by_sei_unknown_sei_is_exists_false():
    body = json.dumps([{"sei": "loomweave:eid:nope", "exists": False}])
    views = _client(FakeTransport([Response(status=200, body=body)])).batch_get_by_sei(["loomweave:eid:nope"])
    assert views is not None and len(views) == 1
    assert views[0].sei == "loomweave:eid:nope"
...
def test_batch_get_by_sei_route_absent_is_loud():
    # A 404 (older SEI Loomweave lacks the route) is a loud read-skew, like batch_get's 4xx.
...
def test_batch_get_by_sei_chunks_large_input():
    ...
    r1 = json.dumps([{"sei": "loomweave:eid:a", "exists": False}])
    r2 = json.dumps([{"sei": "loomweave:eid:b", "exists": False}])
    t = FakeTransport([Response(status=200, body=r1), Response(status=200, body=r2)])
    views = _client(t, batch_max=1).batch_get_by_sei(["loomweave:eid:a", "loomweave:eid:b"])
    assert views is not None
    assert [v.sei for v in views] == ["loomweave:eid:a", "loomweave:eid:b"]
    assert len(t.calls) == 2


def test_batch_get_by_sei_malformed_200_body_is_empty():
    # A 2xx with a non-JSON body degrades to [] (defensive: a Loomweave bug, not ours),
...
class RenameStateTransport:
    """A content-routing double that models Loomweave's store across a rename, so the
...
def test_fact_survives_a_rename_via_sei():
    ...
    the wrong key or hit the wrong route. (The live binding-flip leg is Loomweave's own
    ...
    other = client.batch_get_by_sei(["loomweave:eid:something-else"])
