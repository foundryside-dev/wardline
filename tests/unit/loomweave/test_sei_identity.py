...
two orthogonal status axes. Bodies are anchored to Loomweave's normative fixtures
(get-api-v1-capabilities.json / sei-conformance-oracle.json)."""

from __future__ import annotations

from wardline.loomweave.identity import (
    ContentStatus,
    EntityBinding,
    IdentityStatus,
    SeiCapability,
...
# An older SEI-capable Loomweave (sei.supported True) predates the by-sei route, so a
...
def test_taint_store_capability_independent_of_sei_support() -> None:
    # SEI supported but no taint_store block (an older SEI Loomweave) -> read_by_sei False.
...
def test_binding_prefers_sei_when_present() -> None:
    ...
    assert b.binding_key == "loomweave:eid:0123456789abcdef0123456789abcdef"
