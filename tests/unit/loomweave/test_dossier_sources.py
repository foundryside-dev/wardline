"""T4.3 — LoomweaveLinkageProvider + resolve_entity_binding.

The live Loomweave source for the dossier's linkages section. Reads callers/callees
...
Loomweave, an unknown entity, or an outage yields an honest unavailable section.
"""
...
def test_linkages_compose_callers_and_callees_fresh_live() -> None:
    ...
    sec = LoomweaveLinkageProvider(client, linkages_http=True).linkages(_ALIVE)
...
def test_linkages_unavailable_when_loomweave_lacks_http_route() -> None:
    sec = LoomweaveLinkageProvider(_FakeClient(), linkages_http=False).linkages(_ALIVE)
...
def test_linkages_unavailable_when_both_sides_soft_fail() -> None:
    # entity unknown to Loomweave / outage on both sides → honest unavailable, no crash
    sec = LoomweaveLinkageProvider(_FakeClient(callers=None, callees=None), linkages_http=True).linkages(_ALIVE)
    assert sec.available is False
...
def test_orphaned_identity_axis_is_carried_into_linkages() -> None:
    orphaned = EntityBinding(locator="python:function:old", sei="loomweave:eid:x", identity=IdentityStatus.ORPHANED)
    client = _FakeClient(
        callers=LinkageResult(neighbours=(), total=0, truncated=False),
        callees=LinkageResult(neighbours=(), total=0, truncated=False),
    )
    sec = LoomweaveLinkageProvider(client, linkages_http=True).linkages(orphaned)
...
def test_resolve_entity_binding_none_when_qualname_unresolvable() -> None:
    client = _FakeClient(resolved={})  # qualname not known to Loomweave
