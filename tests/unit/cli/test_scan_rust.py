...
``RS-WL-108`` injection, a clean tree exits 0, the posture banner prints to stderr, and a
...
def test_scan_lang_rust_gate_trips_on_injection(tmp_path) -> None:
    ...
    # The posture banner is loud on stderr (slice coverage + no severity-override parity).
    assert "command-injection slice" in result.output
    assert "severity overrides do not yet apply" in result.output
...
def test_scan_default_lang_python_ignores_rs(tmp_path) -> None:
    # Without --lang rust, a .rs file is not swept and no Rust posture banner prints.
    (tmp_path / "m.rs").write_text(_INJECTION, encoding="utf-8")
    out = tmp_path / "findings.jsonl"
    result = CliRunner().invoke(scan, [str(tmp_path), "--output", str(out)])
    assert result.exit_code == 0
    assert "command-injection slice" not in result.output
