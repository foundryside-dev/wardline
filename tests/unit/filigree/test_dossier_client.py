...
Loomweave's resolve) against the binding's current content hash to set per-ticket DRIFT
...
def test_associations_become_tickets_fresh_when_hash_matches() -> None:
    body = _rows(
        {"issue_id": "wardline-1", "loomweave_entity_id": "loomweave:eid:abc", "content_hash_at_attach": "current-hash"}
    )
    ...
    url = t.calls[0][0]
    assert "entity_id=loomweave%3Aeid%3Aabc" in url
...
def test_scan_results_url_is_normalized_to_api_origin_for_associations() -> None:
    ...
    FiligreeWorkProvider("http://filigree.example/api/weft/scan-results", transport=t).work(_BINDING)
...
def test_unknown_compare_is_unknown_not_fresh_when_binding_hash_absent() -> None:
    ...
    binding = EntityBinding(locator="x", sei="loomweave:eid:abc", identity=IdentityStatus.ALIVE, content_hash=None)
...
def test_urllib_transport_get_surfaces_http_error_status(monkeypatch) -> None:
    ...
    # loomweave's transport), NOT raised as an outage — so work() classifies it by band.
    import io
    import urllib.error
    import urllib.request
