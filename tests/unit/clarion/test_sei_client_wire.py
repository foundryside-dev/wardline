...
def test_capabilities_gets_route_and_parses() -> None:
    ...
    expected = sign_request(
        "s3cr3t", "GET", "/api/v1/_capabilities", sent_body, timestamp=headers["X-Wardline-Timestamp"]
    )
...
def test_resolve_identity_posts_locator_and_signs() -> None:
    ...
    expected = sign_request(
        "s3cr3t", "POST", "/api/v1/identity/resolve", sent_body, timestamp=headers["X-Wardline-Timestamp"]
    )
    assert headers["X-Loom-Component"] == f"clarion:{expected}"
