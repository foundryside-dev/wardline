...
_ANCHORS: tuple[tuple[str, int, str], ...] = (
    ...
    ("src/wardline/core/run.py", 603, "Baseline(frozenset())"),
    ("src/wardline/core/run.py", 634, "def apply_delta_scope"),
    ("src/wardline/core/run.py", 694, "active=sum"),
    ("src/wardline/core/run.py", 795, "honors_suppressions"),
    # src/wardline/cli/scan.py — CLI summary line + gate stderr
    ("src/wardline/cli/scan.py", 667, "suppressed"),
    ("src/wardline/cli/scan.py", 675, "{s.active} active"),
    ("src/wardline/cli/scan.py", 737, "gate: FAILED"),
    # src/wardline/mcp/server.py — MCP scan summary + gate block
    ("src/wardline/mcp/server.py", 949, '"total": result.summary.total'),
    ("src/wardline/mcp/server.py", 950, '"active": result.summary.active'),
    ("src/wardline/mcp/server.py", 951, '"baselined": result.summary.baselined'),
    ("src/wardline/mcp/server.py", 952, '"waived": result.summary.waived'),
    ("src/wardline/mcp/server.py", 953, '"judged": result.summary.judged'),
    ("src/wardline/mcp/server.py", 958, '"informational": result.summary.informational'),
    ("src/wardline/mcp/server.py", 962, '"unanalyzed": result.summary.unanalyzed'),
    ("src/wardline/mcp/server.py", 965, '"gate": {'),
    ("src/wardline/mcp/server.py", 966, '"tripped": decision.tripped'),
    ("src/wardline/mcp/server.py", 971, '"verdict": decision.verdict'),
...
_MAPPING_TABLE_ANCHORS: tuple[tuple[str, str, int, str], ...] = (
    ...
    ("every finding", "src/wardline/mcp/server.py", 949, '"total": result.summary.total'),
    ("every finding", "src/wardline/core/agent_summary.py", 129, '"total_findings"'),
    ("live defect", "src/wardline/cli/scan.py", 675, "{s.active} active"),
    ("live defect", "src/wardline/core/run.py", 104, "active: int"),
    ("live defect", "src/wardline/core/run.py", 694, "active=sum"),
    ("live defect", "src/wardline/mcp/server.py", 950, '"active": result.summary.active'),
    ...
    ("suppressed (sum)", "src/wardline/cli/scan.py", 667, "suppressed"),
    ...
    ("baselined", "src/wardline/mcp/server.py", 951, '"baselined": result.summary.baselined'),
    ("baselined", "src/wardline/core/agent_summary.py", 133, '"baselined"'),
    ("waived", "src/wardline/core/run.py", 107, "waived: int"),
    ("waived", "src/wardline/mcp/server.py", 952, '"waived": result.summary.waived'),
    ("waived", "src/wardline/core/agent_summary.py", 134, '"waived"'),
    ("judged", "src/wardline/core/run.py", 108, "judged: int"),
    ("judged", "src/wardline/mcp/server.py", 953, '"judged": result.summary.judged'),
    ...
    ("under-scan", "src/wardline/mcp/server.py", 962, '"unanalyzed": result.summary.unanalyzed'),
    ...
    ("gate verdict", "src/wardline/mcp/server.py", 965, '"gate": {'),
    ("gate verdict", "src/wardline/mcp/server.py", 966, '"tripped": decision.tripped'),
    ("gate verdict", "src/wardline/mcp/server.py", 971, '"verdict": decision.verdict'),
