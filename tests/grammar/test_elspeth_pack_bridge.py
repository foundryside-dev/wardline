"""Pack-bridge acceptance: binding elspeth's @trust_boundary vocabulary to wardline.

The fix for the elspeth inert-gate (wardline-bd9d1e65cb): rather than re-annotate
elspeth in wardline's vocabulary, a pack maps elspeth's existing
``@trust_boundary(tier=3, source_param=...)`` decorator to a wardline BoundaryType. This
proves the bridge end-to-end on an elspeth-shaped target:
...
from grammar.fixtures.elspeth_trust_boundary_pack import GRAMMAR  # type: ignore[import-not-found]
...
_TARGET = _FIX / "target_elspeth_boundary.py"
...
def test_pack_recognizes_elspeth_boundaries_and_fires_on_leak() -> None:
...
def test_without_pack_elspeth_vocab_is_unrecognized() -> None:
    findings = _scan(None)
    # Default grammar does not read elspeth's vocabulary -> zero boundaries, no defect.
